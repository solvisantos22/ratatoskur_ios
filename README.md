...
- **Skila til kennara** captures all current pages of the assigned exercise and asks for confirmation before sharing them. A timestamped receipt appears only after successful server storage; reopening restores it. This action does not call AI. Later edits require **Skila aftur til kennara**, and a failed pending snapshot can be retried without creating duplicate submissions.
- The teacher can see direct submission snapshots separately from AI attempts and tutor feedback for assigned exercises. Submitted does not mean correct or completed. Unsaved/in-progress local ink is not automatically sent to the teacher. Personal notebooks are excluded from the teacher dashboard.
...
5. Join a class using its code, open an assignment and write with Apple Pencil. Use **Skila til kennara**, confirm, then refresh the teacher browser and inspect the submitted image/time. Close/reopen the notebook to check the local draft and receipt. If the real AI key is configured, also submit a hint/check and inspect its separate feedback.
